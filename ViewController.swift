...
// To interact with reminders, calendars and events
import EventKitUI
import SwiftUI
...
class ViewController: UIViewController, CLLocationManagerDelegate, EKEventViewDelegate {
    
//    func VStack() {
//        addTap()
//    }
    
    let store = EKEventStore()  // events are written to and read from
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(barButtonSystemItem: .add, target: self, action: #selector(addTap))

        ScrollView(.horizontal){
            // scroll to see rest of 7 day forecast when horizontal
        }
    ...
    func layoutViews() {
        ...
        minTemp.topAnchor.constraint(equalTo: tempSymbol.bottomAnchor, constant: 50).isActive = true    //80 -> 50 moves closer to top
    ...
    // added function for events
    @objc func addTap() {
        // Grants access to Calendar
        // [weak self] so it does not cause a memory leak w/ retained cycle
        store.requestAccess(to: .event) { [weak self] success, error in
            if success, error == nil {
                DispatchQueue.main.async {
                    ...
                    newEvent.title = "Senior Presentation"
                    newEvent.startDate = Date()
                    newEvent.endDate = Date()
                    
//                  This section of code allows for the event to be edited (Time, Date, etc.)
    ...
    // function needed for events
    func eventViewController(_ controller: EKEventViewController, didCompleteWith action: EKEventViewAction) {
        controller.dismiss(animated: true, completion: nil) //for when done
    }
